...
## 1. Stack Blueprint (v3 — Python native)

| Layer | Choice | Notes |
|---|---|---|
| Compute | Cloudflare Workers (Python via Pyodide, GA) | Free tier: 100k req/day, **10ms CPU/invocation** |
| Framework | **FastAPI** (Python, via workers ASGI adapter) | Router + middleware + Pydantic validation |
| Validation | **Pydantic v2** | Every request boundary is a Pydantic model |
| Templates | **Jinja2** server-rendered fragments | Auto-escaping, compiled at module scope |
...
| Database | **Cloudflare D1** (serverless SQLite) | Raw SQL prepared statements; migrations via `pywrangler d1 migrations` |
| Cache | **Cloudflare KV** | **Cache only. Never a source of truth. Never sessions.** (eventual consistency ≤ 60s) |
| Sessions | **Stateless HMAC-signed cookies** (Python stdlib `hmac` + `hashlib`) | `HttpOnly; Secure; SameSite=Lax`; short TTL + rolling refresh |
...
| Quality gate | **Ruff** (lint + format) + **Pyright** (type checker) | Agents never hand-format; tools do |
| BDD | **pytest-bdd + Playwright** against `pywrangler dev` | The only test layer. No unit tests. |
| CI/CD | GitHub Actions → `pywrangler deploy` | See `docs/TESTING_AND_QUALITY_SPEC.md` |
...
├── pyproject.toml           # deps, scripts: dev, check, test, deploy
├── src/
│   ├── entry.py             # WorkerEntrypoint → asgi.fetch(app, request, env) — NO logic
│   ├── app.py               # FastAPI app: mount middleware + routes ONLY (no logic)
│   ├── routes/              # 1 file per resource: items.py, auth.py
│   ├── templates/
│   │   ├── layout.html      # full-page shell (rendered ONLY on non-HX requests)
│   │   ├── item_list.html   # list fragment (hx-target="#item-list")
│   │   └── item_row.html    # row fragment (hx-target="#item-{id}")
│   ├── templates.py         # module-scoped Jinja2 environment
│   ├── db/                  # 1 file per table: items.py (typed query fns, raw SQL)
│   ├── schemas/             # Pydantic models: items.py (shared by routes+tests)
│   └── lib/                 # session.py, csrf.py
├── migrations/              # 0001_init.sql ... (pywrangler d1 migrations)
├── features/                # BDD ground truth
│   ├── *.feature
│   ├── steps/               # test_items_archive.py (pytest-bdd step definitions)
│   └── conftest.py          # pywrangler dev lifecycle, browser setup, auth bypass
├── public/                  # htmx.min.js, alpine.min.js, app.css, icons
...
[Browser DOM]
   ...
   ▼
[Cloudflare Worker: FastAPI via ASGI adapter]
   │  middleware: session verify (HMAC cookie) → CSRF (Origin check) → Pydantic validator
   ▼
[db/items.py]  prepared statement → D1 (batch when >1 statement)
   ▼
[templates/item_row.html]  Jinja2 fragment render
   ▼
[htmx swap]  hx-target + hx-swap replaces exactly one DOM node
```

Rules encoded by this flow:
- **Routes contain no business logic**; they orchestrate `schemas → db → templates`.
...
- **Full page vs fragment**: branch on the `HX-Request` header; a GET hit
...
| Form validation UX (pre-submit hints) | Alpine (server re-validates with Pydantic, always) |
...
4. **Module scope = free init.** Hoist constants, Jinja2 environments, Pydantic models to module
   top level. Never construct them per request.
5. **KV fragment cache** for expensive shared reads (`lib/cache.py`, TTL 60–300s), keyed
...
- Rate limiting: none on free tier by default; add a signed-cookie + IP heuristic guard on
  auth endpoints (`lib/oauth.py`) before public launch.
